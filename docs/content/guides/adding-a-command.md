---
title: "Add a command"
description: "Model a real npm record and expose it as a command, a route, and a tool at once."
weight: 10
---

The scaffold ships one example type, `page`. Real work means modelling the
records npm actually serves. You do that in two files, and every surface
updates itself.

## 1. Model the record

In `npm/npm.go`, add a struct for the thing you are fetching
...
type Item struct {
    ...
    Owner string `json:"owner" kit:"link,kind=npm/user"` // an edge to another record
}
...
- `kit:"link,kind=<scheme>/<type>"` marks an outbound edge. It can point at
  another npm type or at another site entirely, which is what lets a
  host walk the graph across tools.

## 2. Declare the operation

In `npm/domain.go`, add an input struct and a handler, then register
it in `Register`:
...
```bash
npm item <id>                 # the command
curl 'localhost:7777/v1/item/<id>'      # the route, under serve
ant get npm://item/<id>       # the URI dereference, via a host
```
