// Package cli assembles the npm command tree from the npm
...
// NewApp assembles the kit application from the npm domain. The
...
//
// To add a command, declare it in npm/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := npm.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(npm.Domain{}).Register(app)
